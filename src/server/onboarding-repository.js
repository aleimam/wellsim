  ...
  return Object.freeze({
    async ready() {
      const result = await call(`SELECT
        ...
        AND NOT has_any_column_privilege(current_user,'app.workspace_invitation','UPDATE')
        AND (SELECT bool_and(p.prosecdef AND p.proowner =
          (SELECT relowner FROM pg_class WHERE oid='app.engineering_case'::regclass)
          AND p.proconfig @> ARRAY['search_path=pg_catalog']
          AND NOT has_function_privilege(current_user,p.oid,'EXECUTE')
          AND NOT has_function_privilege('public',p.oid,'EXECUTE')) FROM pg_proc p
          WHERE p.oid=ANY(ARRAY[
            'app.onboarding_command_v5(text,text,uuid,jsonb)'::regprocedure,
            'app.auth_session_has_recent_mfa(text)'::regprocedure]::oid[]))
        AS safe`, []);
      if (result.rows[0]?.safe !== true) throw new Error('Unsafe onboarding privileges');
    },
    async signIn(identity, hash, csrf, previousHash) {
      return (await call('SELECT app.auth_complete_login($1,$2,$3,$4,$5,$6,$7,$8::bigint,NULL,true) AS id',
        [identity.issuer, identity.subject, identity.email, identity.emailVerified === true,
          hash, csrf, previousHash, identity.mfaAuthenticatedAt ?? null])).rows[0]?.id;
    },
