...
FILE="$APP_DIR/db/migrations/0007_portals_help_and_join_requests.sql"
...
DO $guard$
BEGIN
  IF current_database() NOT IN ('bldrz','bldrz_pool_probe','bldrz_portal_probe') OR
    (SELECT array_agg(version ORDER BY version) FROM app.schema_migration) IS DISTINCT FROM
      ARRAY['0001_platform_foundation','0002_tenant_isolation','0003_personal_workspace_integrity',
        '0004_verified_sessions','0005_controlled_onboarding','0006_administrator_mfa']::text[] THEN
    RAISE EXCEPTION 'portal migration requires an approved bldrz database and exact 0001-0006 baseline';
  END IF;
END
$guard$;
SQL
