...
test -f "$APP_DIR/db/migrations/0005_controlled_onboarding.sql"
admin() {
...
expected='0001_platform_foundation,0002_tenant_isolation,0003_personal_workspace_integrity'
...
admin pg_dump --data-only --lock-wait-timeout=5s --table=app.schema_migration \
  ...
  admin psql -X -q -v ON_ERROR_STOP=1 -d "$PROBE_DB"
for migration in 0004_verified_sessions 0005_controlled_onboarding; do
  { printf 'SET ROLE bldrz_migration_owner;\n';
    sed 's/\<wellsim_runtime\>/bldrz_runtime/g' "$APP_DIR/db/migrations/$migration.sql";
  } | admin psql -X -q -v ON_ERROR_STOP=1 -d "$PROBE_DB"
done
