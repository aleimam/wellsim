...
**Live production revision:** 5 September 2026 — `27ecef0` from
`merge/gas-forecast-into-v2`, deployed 7 September 16:22:29 UTC, asset stamp
`2026-09-07a`.
...
engine to 9.8e-16. It replaced `697dfde` (6 September 13:46:45 UTC); the
rollback archive of that tree is
`/root/wellsim-app-pre-deploy-20260907-162223.tar.gz` — restore with
...
337 tests passing and 43/43 validation sweep. The separate `bldrz`
...
`merge/gas-forecast-into-v2` at `27ecef0` (167 commits), pushed to origin and
in sync, and **this exact tip is what production runs** — the site was
redeployed on 7 September at 16:22:29 UTC. It is ahead of both `origin/main` (`de2393c`) and
...
`main` (`de2393c`) and production have DIVERGED by 88 commits — production and
...
```bash
npm test                          # 337 unit, regression and security tests
